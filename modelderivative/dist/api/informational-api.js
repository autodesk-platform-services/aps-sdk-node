...
class InformationalApi extends base_1.BaseAPI {
    ...
    formats(accessToken, ifModifiedSince, acceptEncoding, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const request = yield (0, exports.InformationalApiFp)(this.sdkManager).formats(accessToken, ifModifiedSince, acceptEncoding, options);
                ...
                if (error.response) {
                    ...
                    throw new base_1.ModelDerivativeApiApiError(`formats Request failed with status : ${error.response.status} and error message: ${error.response.data.reason}`, error);
                }
                else if (error.request) {
                    this.logger.logError(`formats Request failed with no response received: ${error.request}`);
                    throw new base_1.ModelDerivativeApiApiError(`formats Request failed with no response received: ${error.request}`, error);
                }
                throw error;
            }
        });
