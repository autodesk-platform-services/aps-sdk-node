...
// namespace Autodesk.Oss{
  ...
  export class OSSFileTransfer extends BaseAPI implements IOSSFileTransfer{
    ...
    public async Upload( bucketKey: string, objectKey: string, sourceToUpload: Buffer, accessToken: string, cancellationToken: AbortController, projectScope: string = '',
      ...
      const retryCount: number = this._configuration.GetRetryCount();
      ...
      var completeResponse = await this._ossApi.completeSignedS3Upload(
        accessToken,
        bucketKey,
        objectKey,
        "application/json",
    ...
    private async GetS3SignedDownloadUrlWithRetry(bucketKey :string,objectKey:string,accessToken:string,requestId:string,projectScope:string):Promise<Signeds3downloadResponse>{
      ...
      do{
        ...
        try {
          ...
          var response = await this._ossApi.signedS3Download(accessToken,bucketKey,objectKey,projectScope);
